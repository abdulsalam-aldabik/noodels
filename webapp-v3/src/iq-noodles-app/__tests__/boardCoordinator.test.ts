...
describe("BoardCoordinator", () => {
  ...
  test("computes board geometry constants", () => {
    ...
    expect(coordinator.boardPadding).toBe(20);
    expect(coordinator.boardSize).toBe(404);
    expect(coordinator.halfBoard).toBe(202);
  });
  ...
  test("converts row/col to board point and back", () => {
    ...
    expect(boardPoint).toEqual({ x: 244, y: 188 });
  ...
  test("maps DOM coordinates into board space", () => {
    const point = coordinator.domToBoardPoint(300, 250, 100, 50, 404, 404);
