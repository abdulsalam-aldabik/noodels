# Full Rebuild / New Implementation Handoff (Claude -> Copilot)

Paste this whole prompt into Claude Code when starting on a branch where the feature is missing.

Terminology note: in this kit, "rewrite" and "rebuild" mean a new implementation from a missing or untrusted baseline.

---

You are starting a full reimagined implementation (full rebuild) of the vision feature.

## Runtime Invocation (Important)

If this repository's custom Claude assets are available:
1. Run /vision-handoff first.
2. Use the vision-planner-opus agent for planning output.
3. Use the vision-implementer agent for implementation phases.

If slash commands or custom agents are unavailable in this environment, continue with the same workflow manually and state that limitation explicitly.
